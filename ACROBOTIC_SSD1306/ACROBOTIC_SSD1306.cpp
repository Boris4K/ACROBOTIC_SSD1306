...
void ACROBOTIC_SSD1306::setFont(const uint8_t* font)
{
  ...
  m_font_height = pgm_read_byte(&m_font[1]);
}
...
void ACROBOTIC_SSD1306::myClearDisplay()
{
  unsigned char j;
...
void ACROBOTIC_SSD1306::myClearLine(unsigned char row)
{
    setTextXY(row,0);    
    putString("                 ");
}
...
bool ACROBOTIC_SSD1306::myPutChar(unsigned char ch)
{
    if (!m_font) return 0;
    //Ignore non-printable ASCII characters. This can be modified for
    //multilingual font.  
    ...
    for(unsigned char i=0;i<2*m_font_width;i+=2)
    {
       // Font array starts at 0, ASCII starts at 32
      sendData(pgm_read_byte(&m_font[(ch-32)*m_font_width*2+m_font_offset+i]));
    }
    setTextXY(m_row+1,m_col);
    for(unsigned char i=1;i<2*m_font_width;i+=2)
    {
       // Font array starts at 0, ASCII starts at 32
      sendData(pgm_read_byte(&m_font[(ch-32)*m_font_width*2+m_font_offset+i]));
    }
    setTextXY(m_row-1,m_col+1);
}

void ACROBOTIC_SSD1306::myPutString(const char *string)
{
    unsigned char i=0;
    while(string[i])
    {
